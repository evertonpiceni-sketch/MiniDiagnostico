...
import { getTieBreakQuestion, getTiedResults, OPCOES_RESPOSTA, PERGUNTAS, type QuizResult } from './data';
...
export default function App() {
  const [currentStep, setCurrentStep] = useState<'inicio'|'quiz'|'desempate'|'paywall'|'resultado'|'loading'>(() => window.location.pathname === '/resultado' ? 'resultado' : window.location.pathname === '/paywall' ? 'paywall' : 'inicio');
  ...
  const fetchTechnicalResult = async (sessionId: string) => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Não foi possível carregar o resultado de teste.');
  ...
  useEffect(() => {
    ...
    if (window.location.pathname === '/resultado' && sessionId) {
      ...
      if (adminTest) void fetchTechnicalResult(sessionId);
      else if (token) void fetchResult(sessionId, token);
  ...
  const finishQuiz = async (answers:Record<number,number>, desempate?:QuizResult) => {
    setCurrentStep('loading');
    try {
      const res = await fetch('/api/quiz', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({nome:nome.trim(), whatsapp, respostas:answers, desempate}) });
      const data = await res.json().catch(() => ({}));
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex < PERGUNTAS.length - 1) setCurrentQuestionIndex(i => i + 1);
      ...
      const tied = getTiedResults(respostas);
      if (tied.length > 1) { setTieBreakResult(null); setCurrentStep('desempate'); }
  ...
  return <div className="brand-shell min-h-screen flex flex-col items-center justify-center text-stone-800 font-sans"><Toaster position="top-center"/><AnimatePresence mode="wait">
    ...
    {currentStep === 'desempate' && (()=>{const question=getTieBreakQuestion(getTiedResults(respostas));return question&&<motion.main key="desempate" initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} className="journey-page"><section className="brand-page-card quiz-card tie-break-card"><div className="brand-card-header"><img src="/ja-logo-approved.webp" alt="Janaína Araújo"/><span>Pergunta complementar</span><Menu/></div><div className="progress-track"><div className="progress-fill" style={{width:'100%'}}/></div><button type="button" onClick={()=>setCurrentStep('quiz')} className="brand-back"><ArrowLeft/> Voltar</button><p className="brand-eyebrow">PARA DEFINIR SEU RESULTADO</p><h2>{question.texto}</h2><div className="answer-list">{question.opcoes.map((op,index)=><button key={op.resultado} type="button" onClick={()=>setTieBreakResult(op.resultado)} aria-pressed={tieBreakResult===op.resultado} className="answer-option"><span>{String.fromCharCode(65+index)}</span>{op.label}</button>)}</div><div className="journey-actions"><button type="button" onClick={()=>setCurrentStep('quiz')} className="secondary-action"><ArrowLeft/> Voltar</button><button type="button" onClick={()=>tieBreakResult?void finishQuiz(respostas,tieBreakResult):toast.error('Selecione uma resposta para continuar.')} className="primary-action">Ir para pagamento <ArrowRight/></button></div><p className="brand-card-quote">Sua resposta define qual padrão será aprofundado.</p></section></motion.main>})()}
    ...
    {currentStep === 'paywall' && <motion.main key="paywall" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="journey-page"><section className="brand-page-card payment-card"><div className="brand-card-header"><img src="/ja-logo-approved.webp" alt="Janaína Araújo"/><Menu/></div><h2>Finalizar pagamento</h2><p className="payment-intro">Seu relatório personalizado será liberado após a confirmação do pagamento.</p><div className="payment-product"><div><strong>Mini Diagnóstico Emocional</strong><small>Relatório completo + orientações terapêuticas</small></div><b>R$ 9,90</b></div><p className="payment-method-title">Escolha a forma de pagamento</p><div className="flex bg-stone-100 p-1.5 rounded-xl mb-6 gap-1"><button id="tab-pix" type="button" onClick={()=>setActivePaymentTab('pix')} className={`flex-1 py-3 rounded-lg ${activePaymentTab==='pix'?'payment-tab-active bg-white':''}`}><Smartphone className="inline w-4 h-4 mr-2"/>Pix<small>Aprovação imediata</small></button><button id="tab-card" type="button" onClick={()=>setActivePaymentTab('card')} className={`flex-1 py-3 rounded-lg ${activePaymentTab==='card'?'payment-tab-active bg-white':''}`}><CreditCard className="inline w-4 h-4 mr-2"/>Cartão de crédito<small>Ambiente seguro</small></button></div>{activePaymentTab==='pix' && <div className="space-y-4 text-left">{!pixData ? <><div className="payment-security-note">Pix seguro via Asaas. O QR Code e o Copia e Cola são gerados para esta sessão e a confirmação é automática.</div><label className="block text-sm font-medium">CPF do pagador<input value={pixCpf} onChange={e=>setPixCpf(e.target.value)} inputMode="numeric" maxLength={14} className="w-full border rounded-lg px-4 py-3 mt-1" placeholder="000.000.000-00"/></label><button id="btn-pagar-pix-asaas" type="button" disabled={pixLoading} onClick={generatePix} className="w-full py-4 bg-emerald-700 text-white font-bold rounded-xl">{pixLoading?<Loader2 className="inline w-5 h-5 animate-spin"/>:'GERAR PIX — R$ 9,90'}</button></> : <div className="space-y-4 text-center"><div className="font-bold">PIX • R$ 9,90</div><img src={`data:image/png;base64,${pixData.encodedImage}`} alt="QR Code Pix Asaas" className="w-56 h-56 mx-auto bg-white p-3 rounded-xl"/><textarea readOnly value={pixData.payload} className="w-full min-h-24 border rounded-xl p-3 text-xs"/><button type="button" onClick={copyPix} className="w-full py-4 bg-emerald-700 text-white font-bold rounded-xl">{copied?<><Check className="inline w-4 h-4"/> PIX COPIADO</>:<><Copy className="inline w-4 h-4"/> COPIAR CÓDIGO PIX</>}</button><p className="text-xs text-stone-500">Aguardando confirmação automática do Asaas...</p></div>}</div>}{activePaymentTab==='card' && <div className="space-y-4 text-left"><div className="payment-security-note"><div className="flex justify-between font-semibold"><span>Cartão de crédito</span><span>R$ 9,90</span></div><p className="mt-2 text-stone-600">Você será direcionado ao ambiente seguro do Asaas para informar os dados do cartão.</p></div><button id="btn-pagar-cartao-asaas" type="button" disabled={isCheckoutLoading} onClick={handleCheckout} className="w-full py-4 bg-purple-800 text-white font-bold rounded-xl">{isCheckoutLoading?<><Loader2 className="inline w-5 h-5 animate-spin"/> Abrindo Asaas...</>:<><CreditCard className="inline w-5 h-5"/> CONCLUIR PAGAMENTO — R$ 9,90</>}</button></div>}<p className="payment-footer"><ShieldCheck/> Pagamento e dados protegidos com segurança.</p></section></motion.main>}
    {currentStep==='resultado' && resultado && <motion.div key="resultado" variants={resultadoContainerVariants} initial="hidden" animate="visible" data-technical={isTechnicalResult?'1':'0'} className="brand-page-card report-card w-full max-w-2xl bg-white p-6 md:p-10 rounded-2xl shadow-sm border"><div className="report-brand"><img src="/ja-logo-approved.webp" alt="Janaína Araújo"/><p>Seu Relatório Personalizado</p><small>Mini Diagnóstico Emocional</small></div><motion.h2 variants={resultadoItemVariants} className="text-2xl md:text-3xl font-bold mb-6 text-emerald-800 text-center border-b pb-6">Sua principal área de atenção: {resultado.resultado_dominante}</motion.h2><motion.div variants={resultadoItemVariants} className="space-y-4 text-stone-700 leading-relaxed"><p>Olá, {resultado.nome}.</p><p>Este resultado indica o padrão que mais se destacou nas suas respostas. Ele é uma ferramenta de autoconhecimento e não substitui avaliação, diagnóstico ou acompanhamento de profissionais de saúde.</p>{resultado.resultado_dominante==='MEDO'&&<><p>O medo é um mecanismo de proteção. Quando ocupa espaço demais, pode favorecer excesso de cautela, perfeccionismo e dificuldade para agir diante da incerteza.</p><h3 className="text-xl font-bold text-emerald-800">Seu primeiro movimento</h3><p>Escolha uma decisão pequena que você vem adiando e defina uma ação concreta e segura para as próximas 24 horas.</p></>}{resultado.resultado_dominante==='INSEGURANÇA'&&<><p>A insegurança pode fazer você questionar capacidades que já demonstrou possuir e buscar confirmação externa antes de confiar na própria percepção.</p><h3 className="text-xl font-bold text-emerald-800">Seu primeiro movimento</h3><p>Escolha uma pequena decisão e pergunte: “O que eu faria agora se confiasse um pouco mais na minha capacidade?”</p></>}{resultado.resultado_dominante==='PROCRASTINAÇÃO'&&<><p>A procrastinação muitas vezes aparece como uma forma de evitar desconforto, incerteza ou exposição, e não simplesmente como falta de disciplina.</p><h3 className="text-xl font-bold text-emerald-800">Seu primeiro movimento</h3><p>Reduza uma tarefa adiada à menor ação concreta que possa ser feita em dez minutos ou menos.</p></>}</motion.div><motion.div variants={resultadoItemVariants} className="report-actions">{quizSessionId&&(isTechnicalResult||resultToken)&&<a href={isTechnicalResult?'#':`/api/diagnostico-pdf?id=${encodeURIComponent(quizSessionId)}&token=${encodeURIComponent(resultToken)}`} className="report-download">BAIXAR MEU DIAGNÓSTICO EM PDF</a>}<p>Mais do que um resultado, este é um convite para ampliar seu autoconhecimento.</p><div className="therapist-contact"><h3>Aprofunde seu diagnóstico com Janaína</h3><p>Se este resultado fez sentido para você, converse diretamente com a terapeuta.</p><a id="resultado-whatsapp-cta" href={`https://wa.me/5521983928113?text=${encodeURIComponent(`Olá, Janaína! Quero aprofundar meu diagnóstico de ${resultado.resultado_dominante}.`)}`} target="_blank" rel="noopener noreferrer"><MessageCircle/> FALAR COM JANAÍNA NO WHATSAPP</a><a href="https://instagram.com/eujanainaaraujo" target="_blank" rel="noopener noreferrer"><Instagram/> @eujanainaaraujo</a></div></motion.div></motion.div>}
  </AnimatePresence></div>;
}

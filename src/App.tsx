...
import { ArrowLeft, ArrowRight, Check, Copy, CreditCard, Loader2, LockKeyhole, Menu, MonitorSmartphone, ShieldCheck, Smartphone, Sparkles, X } from 'lucide-react';
...
export default function App() {
  const [currentStep, setCurrentStep] = useState<'inicio'|'quiz'|'preview'|'paywall'|'resultado'|'loading'>(() => window.location.pathname === '/resultado' ? 'resultado' : window.location.pathname === '/paywall' ? 'paywall' : 'inicio');
  const [showLeadForm, setShowLeadForm] = useState(false);
  const [previewResult, setPreviewResult] = useState('Resultado confidencial');
  ...
  const finishQuiz = async (answers:Record<number,number>) => {
    ...
    try {
      ...
      setPreviewResult(String(data.resultado_dominante || 'Resultado identificado'));
      setCurrentStep('preview');
      history.replaceState({}, '', `/?session_id=${encodeURIComponent(data.quiz_session_id)}`);
  ...
  return <div className="brand-shell min-h-screen flex flex-col items-center justify-center text-stone-800 font-sans"><Toaster position="top-center"/><AnimatePresence mode="wait">
    {currentStep === 'inicio' && <motion.main key="inicio" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="approved-landing">
      ...
      <button type="button" className="approved-landing__top-cta" aria-label="Começar mini diagnóstico" onClick={()=>setShowLeadForm(true)}>Começar</button>

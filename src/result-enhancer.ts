...
function enhance() {
  ...
  card.innerHTML = `<div class="approved-artwork-result">
    <object class="approved-artwork" data="${pdf}#toolbar=0&navpanes=0&scrollbar=0&view=FitH" type="application/pdf" aria-label="Resultado ${pattern}">
      <a href="${pdf}" target="_blank" rel="noopener noreferrer">Abrir resultado ${pattern}</a>
    </object>
    <a class="artwork-hotspot artwork-whatsapp" href="https://wa.me/5521983928113?text=${encodeURIComponent(message)}" target="_blank" rel="noopener noreferrer" aria-label="Quero aprofundar meu resultado com Janaína"></a>
    <a class="artwork-hotspot artwork-pdf" href="${downloadUrl}" download="${filename}" aria-label="Baixar meu resultado em PDF"></a>
  </div>`;
}

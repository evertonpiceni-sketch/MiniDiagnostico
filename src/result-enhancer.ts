...
function enhance() {
  ...
  const message = `Olá, Janaína! 🌿\n\nMeu nome é ${name} e acabei de concluir o Mini Diagnóstico. Meu padrão predominante foi ${pattern}.\n\nO resultado trouxe alguns pontos que fizeram bastante sentido para mim e gostaria de compreender melhor o que apareceu no meu diagnóstico e saber como posso aprofundar esse processo com o seu acompanhamento.\n\nVim pelo Mini Diagnóstico — Janaína Araújo. 💜`;
  ...
  if (technical) card.querySelector<HTMLAnchorElement>('.result-pdf')?.addEventListener('click', event => { event.preventDefault(); window.print(); });
}

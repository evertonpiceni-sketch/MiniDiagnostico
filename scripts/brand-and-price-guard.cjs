...
for (const fragment of [
  ...
  '/ja-logo.webp', '/hero-approved-repaired.webp', 'Resultado confidencial', 'Asaas'
...
if (!app.includes('<img src="/hero-approved-repaired.webp"')) throw new Error('Production guard failed: validated approved hero element missing');
...
const hero = fs.readFileSync('public/hero-approved-repaired.webp');
if (hero.length < 50_000 || hero.subarray(0, 4).toString('ascii') !== 'RIFF' || hero.subarray(8, 12).toString('ascii') !== 'WEBP') {
  throw new Error('Production guard failed: repaired approved hero WebP is missing or corrupt');
}

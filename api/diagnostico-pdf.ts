...
function makePdf(lines: string[]) {
  ...
  const content = ['q','90 0 0 90 50 735 cm','/Im1 Do','Q','BT','/F1 18 Tf','165 770 Td','(JANAINA ARAUJO) Tj','/F1 10 Tf','0 -18 Td','(MINI DIAGNOSTICO) Tj','ET','BT','/F1 16 Tf','50 650 Td',`(${safeLines[0]}) Tj`,'/F1 11 Tf',...safeLines.slice(1).flatMap(line => ['0 -24 Td',`(${line}) Tj`]),'ET'].join('\n');
  ...
  objects.forEach((object, index) => { const prefix = Buffer.from(`${index + 1} 0 obj\n`, 'latin1'); const suffix = Buffer.from('\nendobj\n', 'latin1'); offsets[index + 1] = position; chunks.push(prefix, object, suffix); position += prefix.length + object.length + suffix.length; });
...
export default async function handler(req: Req, res: Res) {
  ...
  try {
    ...
    res.status(200); res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', `attachment; filename="${filename}"`); res.setHeader('Cache-Control', 'private, no-store'); res.end(pdf);
  } catch (e: any) {

...
function makePdf(lines: string[]) {
  ...
  const stream = [
    'BT',
    '/F1 20 Tf', '50 760 Td', `(${pdfText(safeLines[0])}) Tj`,
    '/F1 11 Tf',
...
function summary(dominant: string) {
  if (dominant === 'INSEGURANÇA') return 'Seu resultado indica maior peso do padrão de inseguranca: observe como duvidas e autocobranca podem influenciar suas decisoes.';
  if (dominant === 'PROCRASTINAÇÃO') return 'Seu resultado indica maior peso do padrão de procrastinacao: observe como a urgencia pode estar sendo usada como gatilho para agir.';
  return 'Seu resultado indica maior peso do padrão de medo: observe como receios e antecipacao de consequencias podem influenciar suas decisoes.';
}

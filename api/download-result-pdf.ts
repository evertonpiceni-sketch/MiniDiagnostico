...
function addWhatsAppLink(pdf: PDFDocument, url: string) {
  ...
  const x1 = width * 0.13;
  const x2 = width * 0.87;
  const y1 = height * (1 - 0.79 - 0.056);
  const y2 = height * (1 - 0.79);
  const rect = PDFArray.withContext(pdf.context);
  [x1, y1, x2, y2].forEach(value => rect.push(PDFNumber.of(value)));
